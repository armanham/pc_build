    ...
    public Order save(final Set<ProductDTO> products, final BigDecimal totalPrice, final User user) {
        ...
        for (ProductDTO product : products) {
            if (product.getId() >= INITIAL_ID_VALUE_CASE && product.getId() <= FINAL_ID_VALUE_CASE) {
                order.addCase(caseDAO.findById(product.getId()).orElseThrow(() -> new ProductNotFoundException("aaa")));//todo
            } else if (product.getId() >= INITIAL_ID_VALUE_COOLER && product.getId() <= FINAL_ID_VALUE_COOLER) {
                order.addCooler(coolerDAO.findById(product.getId()).orElseThrow(() -> new ProductNotFoundException("aaa")));
            } else if (product.getId() >= INITIAL_ID_VALUE_CPU && product.getId() <= FINAL_ID_VALUE_CPU) {
                order.addCPU(cpuDAO.findById(product.getId()).orElseThrow(() -> new ProductNotFoundException("aaa")));
            } else if (product.getId() >= INITIAL_ID_VALUE_CPU_COOLER && product.getId() <= FINAL_ID_VALUE_CPU_COOLER) {
                order.addCPUCooler(cpuCoolerDAO.findById(product.getId()).orElseThrow(() -> new ProductNotFoundException("aaa")));
            } else if (product.getId() >= INITIAL_ID_VALUE_GPU && product.getId() <= FINAL_ID_VALUE_GPU) {
                order.addGPU(gpudao.findById(product.getId()).orElseThrow(() -> new ProductNotFoundException("aaa")));
            } else if (product.getId() >= INITIAL_ID_VALUE_INTERNAL_HARD_DRIVE && product.getId() <= FINAL_ID_VALUE_INTERNAL_HARD_DRIVE) {
                order.addInternalHardDrive(internalHardDriveDAO.findById(product.getId()).orElseThrow(() -> new ProductNotFoundException("aaa")));
            } else if (product.getId() >= INITIAL_ID_VALUE_MOTHERBOARD && product.getId() <= FINAL_ID_VALUE_MOTHERBOARD) {
                order.addMotherboard(motherboardDAO.findById(product.getId()).orElseThrow(() -> new ProductNotFoundException("aaa")));
            } else if (product.getId() >= INITIAL_ID_VALUE_POWER_SUPPLY && product.getId() <= FINAL_ID_VALUE_POWER_SUPPLY) {
                order.addPowerSupply(powerSupplyDAO.findById(product.getId()).orElseThrow(() -> new ProductNotFoundException("aaa")));
                ...
                order.addKeyboard(keyboardDAO.findById(product.getId()).orElseThrow(() -> new ProductNotFoundException("aaa")));
            } else if (product.getId() >= INITIAL_ID_VALUE_MONITOR && product.getId() <= FINAL_ID_VALUE_MONITOR) {
                order.addMonitor(monitorDAO.findById(product.getId()).orElseThrow(() -> new ProductNotFoundException("aaa")));
            } else if (product.getId() >= INITIAL_ID_VALUE_MOUSE && product.getId() <= FINAL_ID_VALUE_MOUSE) {
                order.addMouse(mouseDAO.findById(product.getId()).orElseThrow(() -> new ProductNotFoundException("aaa")));
            } else if (product.getId() >= INITIAL_ID_VALUE_SPEAKER && product.getId() <= FINAL_ID_VALUE_SPEAKER) {
                order.addSpeaker(speakerDAO.findById(product.getId()).orElseThrow(() -> new ProductNotFoundException("aaa")));
            } else {
                throw new IllegalArgumentException("range");//todo
            }
        }
